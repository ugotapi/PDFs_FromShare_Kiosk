...
- My scripts assume a username "user" is the user in these scripts. Edit the commands if needed. 
...
- On a reboot the files on the ramdisk are deleted. Only the PDFs in /home/user/slideshow/pdfs are brought up on boot. The next sudo -e crontab will mirror the network files. 
- The working html for Apache is copied over from the /home/user/slideshow folder for Apache to use for the slideshow.
- There is a script that checks to see if Chromium crashes displaying the words "Aw, Snap". If the error messsage says Aw Snap. It stops and restarts Chromium.
- The index.php uses pdf.js from Mozilla https://mozilla.github.io/pdf.js/ to display multipage pdfs and cycle to the next pdf.
